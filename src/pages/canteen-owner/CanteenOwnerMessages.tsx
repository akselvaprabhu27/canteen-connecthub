...
const CanteenOwnerMessages = () => {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)] bg-card rounded-3xl border border-border overflow-hidden shadow-sm animate-fade-in">
      {/* Header */}
      <div className="p-6 border-b border-border/50 bg-muted/30 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="font-black text-foreground">Organization Communication</h3>
            <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest mt-0.5">Official Thread</p>
          </div>
        </div>
      </div>
      ...
      <ScrollArea className="flex-1 p-6">
        <div className="space-y-6">
          {loading ? (
            <div className="py-20 text-center animate-pulse font-bold text-muted-foreground">Loading messages...</div>
          ) : sortedMessages.length === 0 ? (
            <div className="py-20 text-center">
              <MessageSquare className="mx-auto h-12 w-12 text-muted-foreground/20 mb-4" />
              <p className="text-lg font-bold text-foreground">No messages from organization yet</p>
              <p className="text-sm text-muted-foreground mt-1">Official updates and notices will appear here.</p>
            </div>
          ) : (
                ...
                <div key={m._id} className={`flex flex-col ${isMe ? "items-end" : "items-start"}`}>
                  <div className={`max-w-[80%] space-y-1`}>
                    ...
                    <div className={`rounded-2xl p-4 shadow-md relative ${
                      ...
                      <p className={`text-sm font-medium leading-relaxed ${isMe ? "text-white/90" : isSystem ? "text-amber-950/90" : "text-white/80"}`}>{m.message}</p>
                    </div>
                  </div>
                </div>
              );
      ...
      <div className="p-6 bg-muted/30 border-t border-border/50">
        ...
        {messages.length === 0 && (
          <p className="text-[10px] text-center mt-3 font-bold text-muted-foreground uppercase tracking-wider">
